    ...
    async updateRunSummary(record) {
      ...
      const rows = await sql`
        ...
        SET
          ...
          summary_json = ${JSON.stringify(record.summaryJson)}::jsonb,
          updated_at = now()
        WHERE id = ${record.id}
        RETURNING *
      `;
...
export function createRunStore({
  ...
  return {
    async createRunFromUpload({ buffer, filename, contentType }, { source = "webhook" } = {}) {
      ...
      if (existing) {
        const existingRun = rowToRun(existing);
        const refreshedSummary = summarizeFit(buffer, existingRun.file);
        enforceRunningActivity(refreshedSummary);
        const currentSummary = parseSummary(existing.summary_json);
        if (repository.updateRunSummary && JSON.stringify(currentSummary) !== JSON.stringify(refreshedSummary)) {
          const updated = await repository.updateRunSummary({
            id: existing.id,
            summaryJson: refreshedSummary,
            ...activityFieldsFromSummary(refreshedSummary),
          });
          return rowToRun(updated || existing);
        }
        enforceRunningActivity(existingRun);
        return existingRun;
      }
      ...
      enforceRunningActivity(summary);
    ...
    async listAllRuns() {
      const rows = await repository.listAllRuns();
      return rows.map(rowToRun).filter(Boolean);
    },
    async countRuns() {
...
export async function listAllRuns() {
  return getDefaultStore().listAllRuns();
}
...
export function rowToRun(row) {
  ...
  const summary = normalizeStoredSummary(parseSummary(row.summary_json));
  const id = row.id;
  return {
    ...summary,
    run: {
...
function enforceRunningActivity(summary) {
  const activity = summary.details?.activity;
  if (["Running", "Indoor Run"].includes(activity)) return;

  const error = new Error(`Only running FIT files are supported. Received ${activity || "unknown activity"}.`);
  error.status = 422;
  throw error;
}
...
function normalizeStoredSummary(summary) {
  if (summary.details?.activity !== "Running" || summary.route) return summary;
  return {
    ...summary,
    details: {
      ...summary.details,
