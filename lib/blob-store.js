...
export async function getLatestActivity() {
  if (hasBlobToken()) {
    try {
      const summary = await getBlobSummary(CURRENT_SUMMARY_PATH) || await getNewestTimestampedBlobSummary();
      return hydrateMissingRoute(summary);
    } catch (error) {
  ...
  try {
    ...
    if (record.summary) return hydrateMissingRoute(record.summary);
...
async function hydrateMissingRoute(summary) {
  if (!summary || summary.route || !summary.file?.rawPath) return summary;

  if (hasBlobToken()) {
    const blob = await get(summary.file.rawPath, { access: "private", useCache: false });
    if (!blob?.stream) return summary;
    const buffer = Buffer.from(await streamToArrayBuffer(blob.stream));
    return summarizeFit(buffer, summary.file);
  }

  const { readFile } = await localFileHelpers();
  return summarizeFit(await readFile(summary.file.rawPath), summary.file);
}
...
async function streamToText(stream) {
  return new TextDecoder().decode(await streamToArrayBuffer(stream));
}
