...
function buildActivitySummary(parsed, latest) {
  ...
  const subSport = firstNumber(session[6]);
  const isRunning = sport === 1;
  const startTime = firstNumber(session[2], fileId[4], records[0]?.[253]);
  const startTimeIso = startTime ? fitTimestampToIso(startTime) : null;
  ...
  return {
    file: latest,
    details: {
      activity: activityName(sport, subSport),
      date: startTimeIso ? formatDate(startTimeIso) : formatDate(latest.uploadedAt),
      sortDate: startTimeIso,
...
function buildRouteMap(records) {
  const points = records
    .map((record) => {
      ...
      return { latitude, longitude, distance: metersToMiles(distanceMeters) };
    })
    .filter(Boolean);
  ...
  return {
    points: downsampled.map((point) => ({
      ...
      distance: point.distance === null ? null : Number(point.distance.toFixed(3)),
    })),
    start: {
...
function activityName(sport, subSport) {
  if (sport === 1 && subSport === 1) return "Indoor Run";
...
function fitTimestampToIso(seconds) {
  return new Date((seconds + 631065600) * 1000).toISOString();
}
